...
async fn attach_and_collect_device_stats(nvml: &Nvml, index: u32) -> serde_json::Value {
    let device = nvml.device_by_index(index).unwrap();
    let mut stats = serde_json::Map::new();

    // Collect various stats
    stats.insert("name".to_string(), json!(device.name().unwrap()));

    if let Ok(utilization) = device.utilization_rates() {
        stats.insert("gpu_utilization".to_string(), json!(utilization.gpu));
        stats.insert("memory_utilization".to_string(), json!(utilization.memory));
    }

    if let Ok(memory) = device.memory_info() {
        stats.insert("memory_total".to_string(), json!(memory.total));
        stats.insert("memory_used".to_string(), json!(memory.used));
    }

    if let Ok(temperature) =
        device.temperature(nvml_wrapper::enum_wrappers::device::TemperatureSensor::Gpu)
    {
        stats.insert("temperature".to_string(), json!(temperature));
    }

    json!({
        format!("gpu_{}", index): stats
    })
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let futures = (0..device_count).map(|index| attach_and_collect_device_stats(&nvml, index));
    let results = join_all(futures).await;

    let mut combined_stats = serde_json::Map::new();
    for result in results {
        combined_stats.extend(result.as_object().unwrap().clone());
    }

    combined_stats.insert(
        "total_time_ms".to_string(),
        json!(start.elapsed().as_millis()),
    );

    println!("{}", serde_json::to_string_pretty(&combined_stats)?);
